...
class DatasetFsArchive extends EventEmitter {
    ...
    init(dataset) {
        ...
        this.idCursor = parseInt(Object.keys(this.dataset).reduce((acc,cur) => {
            if(cur > acc) {
                acc = cur;
            }
            return acc;
    ...
    async save(callback) {
        ...
        const saveFile = () => { return new Promise((resolve, reject) => {
                fs.writeFile(this.archiveFile, JSON.stringify(this.dataset), (err, data) => {
                    if(err) {
                        reject(err);
                    }
                    else {
...
module.exports = () => { return new DatasetFsArchive() };
